        ...
        <div class="xl:w-full  min-h-[calc(100vh-138px)] relative pb-14">
            <form action="#" method="post" enctype="multipart/form-data">
                <div class="grid grid-cols-12 sm:grid-cols-12 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 gap-4 justify-between">
                    ...
                    <?php extract($loadbt);
                    $dv1=load_one_dv($id_dv);
                    $tc1=load_one_tc($id_loai_dong_vat);
                    $kt1=load_one_kt($id_kich_thuoc);
                            ...
                            <div class="flex-auto p-0 md:p-4">
                                <div class="mb-2">
                                    ...
                                    <select name="id_dv" id="dv"class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700">
                                        <option value="<?=$dv1['id']?>"><?=$dv1['ten_dv']?></option>
                                        <?php  
                                            foreach($dv as $item){
                                                extract($item);
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="id_tc" id="thu_cung" class="w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700">
                                    <option value="<?=$tc1['id']?>"><?=$tc1['ten_loai']?></option>
                                        <?php  
                                            foreach($tc as $item){
                                                extract($item);
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="id_kt" id="can_nang" class="w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700">
                                    <option value="<?=$kt1['id']?>"><?=$kt1['can_nang']?></option>
                                        <?php  
                                            foreach($kt as $item){
                                                extract($item);
                                ...
                                <div class="mb-2">
                                    ...
                                    <input name="gia" type="title" id="title" value="<?=$gia?>" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700" placeholder="Giá" required>
                                </div>


                                <div class="">
                                    <input name="luu" type="submit" value="Lưu" class="px-2 py-2 lg:px-4 bg-brand  text-white text-sm  rounded hover:bg-brand-600 border border-brand-500"></input>
